...
router.post("/getMemberScore", async (req, res) => {
    ...
    try {
      ...
      const dateList = await pool.query(`select distinct TO_CHAR(record_date, 'YYYY-MM-dd') as record_date  from member_score where member_id = $1`, [id])
      res.json({ success: true, result: result.rows, dateList: dateList.rows });
    } catch (err) {
...
router.get("/getSidoList", async (req, res) => {
    try {
      const result = await pool.query(
        `select * from sido where sido_code != '0' order by sido_name`
      );
...
router.get("/getSggList", async (req, res) => {
    const { sidoCode } = req.query;

    try {
      const result = await pool.query(
        `select * from sgg where sgg_code ilike $1 order by sgg_name;`
        , [`${sidoCode}%`]
      );
...
router.get("/getStateList", async (req, res) => {
    const { sidoCd, sggCd } = req.query;

    try {
      const result = await pool.query(
        `select * from archery_place where sgg ilike $1 and sgg ilike $2 order by archery_name`
        , [`${sidoCd}%`, `${sggCd}%`]
      );
...
router.get("/getArcheryList", async (req, res) => {
  const { sidoCd = "", sggCd = "", state = "" } = req.query;
  ...
  let sql = `SELECT  
                ...
                archery_name,
                target_count,
                direction FROM archery_place WHERE 1=1`;
  const values = [];
